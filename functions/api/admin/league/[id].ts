...
const FIELDS = [
  ...
  "location_line", "image_url", "cta_label", "cta_url", "published",
] as const;

export const onRequestGet: PagesFunction<Env> = async ({ request, env, params }) => {
  const user = await requireAdmin(request, env);
  if (user instanceof Response) return user;
  const row = await env.DB.prepare(`SELECT * FROM league_events WHERE id = ?`).bind(params.id).first();
...
export const onRequestPatch: PagesFunction<Env> = async ({ request, env, params }) => {
  ...
  await env.DB.prepare(`UPDATE league_events SET ${updates.join(", ")} WHERE id = ?`)
    .bind(...values)
    .run();
  return json({ ok: true });
};
...
export const onRequestDelete: PagesFunction<Env> = async ({ request, env, params }) => {
  const user = await requireAdmin(request, env);
  if (user instanceof Response) return user;
  await env.DB.prepare(`DELETE FROM league_events WHERE id = ?`).bind(params.id).run();
  return json({ ok: true });
};
