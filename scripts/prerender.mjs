...
//   3. For each route in scripts/routes.mjs: renders the app, strips the
...
//   4. Removes dist-ssr/.
//
// Non-prerendered routes (checkout flows, /admin, program slugs created
// later in the admin panel) are NOT backed by a dedicated fallback file
// here. See the comment block at the bottom of public/_redirects for why:
// two attempts at a custom catch-all rewrite rule both caused a live
// redirect loop in production (2026-07-21). Cloudflare Pages' own default
// SPA behavior (no matching rule + no top-level 404.html) now serves root
// dist/index.html for those paths instead, and src/main.tsx's marker
// check already handles the resulting mismatch with a plain client
// render. Do not reintroduce a custom "/*" rewrite target without testing
// it on a Cloudflare Pages preview deployment first.
//
// Cloudflare Pages behavior this relies on (verified against the live
// deployment): static assets are served in preference to _redirects
// rules, and flat HTML files resolve to extensionless clean URLs
...
// No separate fallback-shell file is written here — see the top-of-file
// note on why (two production redirect-loop incidents, 2026-07-21).
...
// 4. Clean up the SSR bundle — it must not be deployed.
